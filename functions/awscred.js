...
exports.regionCallChain = [loadRegionFromEnv, loadRegionFromIniFile];
...
function loadCredentials(options, cb) {
  ...
  var credentialsCallChain =
    options.credentialsCallChain || exports.credentialsCallChain;
...
function loadCredentialsFromEnv(options, cb) {
  ...
  cb(null, {
    accessKeyId:
      env.AWS_ACCESS_KEY_ID || env.AMAZON_ACCESS_KEY_ID || env.AWS_ACCESS_KEY,
    secretAccessKey:
...
function loadCredentialsFromEcs(options, cb) {
  ...
  cb = function(err, credentials) {
    ecsCallbacks.forEach(function(cb) {
      cb(err, credentials);
    });
    ecsCallbacks = [];
  };
  ...
  return request(options, function(err, res, data) {
    ...
    if (res.statusCode != 200)
      return cb(
        new Error('Failed to fetch IAM role: ' + res.statusCode + ' ' + data)
      );
    ...
    if (res.statusCode != 200)
      return cb(
        new Error(
          'Failed to fetch IAM credentials: ' + res.statusCode + ' ' + data
        )
      );
...
function loadCredentialsFromEc2Metadata(options, cb) {
  ...
  cb = function(err, credentials) {
    ec2Callbacks.forEach(function(cb) {
      cb(err, credentials);
    });
    ec2Callbacks = [];
  };
...
function loadProfileFromIniFile(options, defaultFilename, cb) {
  var filename =
      options.filename || path.join(resolveHome(), '.aws', defaultFilename),
...
function merge(obj, options, cb) {
  ...
  var needCreds =
    !obj.credentials ||
    !obj.credentials.accessKeyId ||
    !obj.credentials.secretAccessKey;

  function loadCreds(cb) {
    if (needRegion && needCreds) {
      return loadCredentialsAndRegion(options, cb);
    } else if (needRegion) {
      return loadRegion(options, function(err, region) {
        cb(err, { region: region });
      });
    } else if (needCreds) {
      return loadCredentials(options, function(err, credentials) {
        cb(err, { credentials: credentials });
      });
    }
    cb(null, {});
  }
    ...
    if (creds.credentials) {
      if (!obj.credentials) {
        ...
        Object.keys(creds.credentials).forEach(function(key) {
          if (!obj.credentials[key])
            obj.credentials[key] = creds.credentials[key];
        });
      }
    }
